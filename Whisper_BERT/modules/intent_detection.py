...
class IntentDetector:
    ...
    def _initialize_ner_model(self):
        ...
        self.using_custom_ner = False
        self.ner_pipeline = None
        
        try:
            # Try to load our fine-tuned NER model for metro booking entities
            custom_model_path = os.path.join(os.getcwd(), "models", "metro_ner")
            
            if os.path.exists(custom_model_path):
                print(f"Loading fine-tuned NER model from: {custom_model_path}")
                
                from transformers import DistilBertForTokenClassification, DistilBertTokenizerFast
                
                self.ner_tokenizer = DistilBertTokenizerFast.from_pretrained(custom_model_path)
                self.ner_model = DistilBertForTokenClassification.from_pretrained(
                    custom_model_path
                ).to(self.device)
                
                # Load label mapping
                labels_path = os.path.join(custom_model_path, "labels.txt")
                if os.path.exists(labels_path):
                    with open(labels_path, "r") as f:
                        self.ner_labels = f.read().splitlines()
                    print(f"Loaded label mapping: {self.ner_labels}")
                else:
                    print(f"Label mapping file not found at {labels_path}, using default labels")
                    self.ner_labels = ["O", "B-FROM", "B-TO", "B-NUM"]
                    
                # Set flag to indicate we're using our custom model
                self.using_custom_ner = True
                
            else:
                # Fall back to generic NER model
                print(f"Custom NER model not found at {custom_model_path}, using generic model")
                ...
                try:
                    ...
                    print(f"Loading generic NER model: {model_id}")
                    ...
                    print(f"Error loading generic NER pipeline: {e}")
                    print("Using rule-based fallback for entity extraction")
                
        except Exception as e:
            print(f"Error during NER model initialization: {e}")
            print("Using rule-based fallback for entity extraction")
    ...
    def detect_intent(self, text):
        ...
        booking_keywords = {
            'en': ['book', 'ticket', 'metro', 'train', 'from', 'to', 'station','for'],
    ...
    def extract_entities(self, text):
        ...
        # Use our fine-tuned model if available
        if hasattr(self, 'using_custom_ner') and self.using_custom_ner:
            try:
                ...
                for token, prediction in zip(tokens, predictions[0]):
                    ...
                    if pred_id < len(self.ner_labels):
                        label = self.ner_labels[pred_id]
                        
                        if token.startswith("##"):
                            # If it's a subword token, append to the current entity
                            if from_station and label == "B-FROM":
                                from_station[-1] += token[2:]
                            elif to_station and label == "B-TO":
                                to_station[-1] += token[2:]
                            elif num_tickets and label == "B-NUM":
                                num_tickets[-1] += token[2:]
                        else:
                            # New token
                            if label == "B-FROM":
                                from_station.append(token)
                            elif label == "B-TO":
                                to_station.append(token)
                            elif label == "B-NUM":
                                num_tickets.append(token)
                
                # Update entities with the extracted information
                if from_station:
                    entities['source'] = " ".join(from_station)
                
                if to_station:
                    entities['destination'] = " ".join(to_station)
                
                if num_tickets:
                    try:
                        entities['tickets'] = int(" ".join(num_tickets))
                    except ValueError:
                        # If conversion fails, keep the default value
                        pass
                
                print(f"Custom NER model extracted: {entities}")
                
                # If we have valid entities from our custom model, we can return now
                if entities['source'] and entities['destination']:
                    return entities
                    
            except Exception as e:
                print(f"Error using custom NER model: {e}")
                print("Falling back to rule-based extraction")
        
        # Fall back to the generic NER pipeline or rule-based extraction
        if self.ner_pipeline is not None:
            ner_results = self.ner_pipeline(text)
            print(f"Generic NER results: {ner_results}")
